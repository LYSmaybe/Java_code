...
public class MyLinkedList2<E> extends AbstractList<E> {
    ...
    private Node<E> node(int index){
        rangeCheck(index);
        Node<E> node = first.next; //此处由于新加入了头节点，所以取真正的值是从first的下一个开始
    ...
    public void add(int index, E element){
        ...
        //此时插入值不再需要分类处理头结点和其余结点了
        Node<E> prev = index == 0 ? first : node(index - 1); //此处增加三目运算符，进行是否为头节点插入的小处理
    ...
    public String toString() {
        ...
        Node<E> node = first.next; //从first的下一节点开始拼接
        for(int i = 0; i < size; i++){
